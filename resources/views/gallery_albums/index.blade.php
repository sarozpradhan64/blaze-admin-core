                ...
                <x-ui.table-body>
                    @forelse($albums as $album)
                        <x-ui.table-row>
                            ...
                            <x-ui.table-cell>
                                <a href="{{ route('admin.gallery-items.index', ['album_id' => $album->id]) }}" class="text-primary hover:underline">{{ $album->items_count }} items</a>
                            </x-ui.table-cell>
                            <x-ui.table-cell>
                                <x-ui.badge variant="{{ $album->status ? 'default' : 'secondary' }}">{{ $album->status ? 'Active' : 'Draft' }}</x-ui.badge>
                            </x-ui.table-cell>
                            <x-ui.table-cell class="text-right">
                                <div class="flex justify-end gap-2">
                                    <x-ui.button variant="ghost" size="icon" href="{{ route('admin.gallery-albums.edit', $album) }}">
                                        <x-lucide-edit class="size-4" />
                                    </x-ui.button>
                                    <form action="{{ route('admin.gallery-albums.destroy', $album) }}" method="POST" onsubmit="return confirm('Are you sure?');">
                                        @csrf @method('DELETE')
                    ...
                    @empty
                        <x-ui.table-row>
                            <x-ui.table-cell colspan="4" class="text-center py-6 text-muted-foreground">No albums found.</x-ui.table-cell>
                        </x-ui.table-row>
                    @endforelse
                </x-ui.table-body>
            </x-ui.table>
