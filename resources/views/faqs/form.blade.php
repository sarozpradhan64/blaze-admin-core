    ...
    <x-slot:header>
        <x-ui.breadcrumb>
            <x-ui.breadcrumb-list>
                <x-ui.breadcrumb-item><x-ui.breadcrumb-link href="{{ route('admin.faqs.index') }}">FAQs</x-ui.breadcrumb-link></x-ui.breadcrumb-item>
                <x-ui.breadcrumb-separator />
                <x-ui.breadcrumb-item><x-ui.breadcrumb-page>{{ isset($faq) ? 'Edit FAQ' : 'Add FAQ' }}</x-ui.breadcrumb-page></x-ui.breadcrumb-item>
            </x-ui.breadcrumb-list>
        </x-ui.breadcrumb>
    </x-slot:header>
            ...
            <x-ui.card>
                <x-ui.card-content class="space-y-6 pt-6">
                    <x-ui.field>
                        ...
                        <x-ui.input id="question" name="question" value="{{ old('question', $faq->question ?? '') }}" autofocus />
                    ...
                    <div class="flex items-center justify-between">
                        <x-ui.label for="status" class="flex flex-col space-y-1">
                            ...
                            <span class="font-normal text-xs text-muted-foreground">Make this FAQ available</span>
                        </x-ui.label>
                        <x-ui.switch id="status" name="status" value="1" :checked="old('status', $faq->status ?? true)" />
                    </div>
                </x-ui.card-content>
                <x-ui.card-footer class="border-t bg-muted/50 flex justify-end p-4 gap-2">
