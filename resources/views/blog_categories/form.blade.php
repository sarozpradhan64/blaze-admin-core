            ...
            <x-ui.breadcrumb-list>
                ...
                <x-ui.breadcrumb-item>
                    <x-ui.breadcrumb-link
                        href="{{ route('admin.blog-categories.index') }}">Categories</x-ui.breadcrumb-link>
                </x-ui.breadcrumb-item>
                <x-ui.breadcrumb-separator />
                <x-ui.breadcrumb-item>
    ...
    <div class="max-w-2xl">
        <div class="mb-6 flex items-center justify-between">
            <h2 class="text-2xl font-bold tracking-tight">
                {{ isset($category) ? 'Edit Blog Category' : 'Add New Blog Category' }}</h2>
        </div>
                ...
                <x-ui.card-content class="space-y-4">
                    ...
                    <x-ui.field>
                        ...
                        <x-ui.textarea id="description" name="description"
                            rows="4">{{ old('description', $category->description ?? '') }}</x-ui.textarea>
                    ...
                    <div class="flex items-center justify-between">
                        <x-ui.label for="status" class="flex flex-col space-y-1">
                            <span>Active</span>
                            <span class="font-normal text-xs text-muted-foreground">Available for new posts</span>
                        </x-ui.label>
                        <x-ui.switch id="status" name="status" value="1" :checked="old('status', $category->status ?? true)" />
                    </div>
                </x-ui.card-content>
                <x-ui.card-footer class="border-t bg-muted/50 flex justify-end gap-2 p-4">
                    <x-ui.button variant="outline"
                        href="{{ route('admin.blog-categories.index') }}">Cancel</x-ui.button>
