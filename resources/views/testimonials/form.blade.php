    ...
    <x-slot:header>
        <x-ui.breadcrumb>
            <x-ui.breadcrumb-list>
                <x-ui.breadcrumb-item><x-ui.breadcrumb-link href="{{ route('admin.testimonials.index') }}">Testimonials</x-ui.breadcrumb-link></x-ui.breadcrumb-item>
                <x-ui.breadcrumb-separator />
                <x-ui.breadcrumb-item><x-ui.breadcrumb-page>{{ isset($testimonial) ? 'Edit' : 'Add' }}</x-ui.breadcrumb-page></x-ui.breadcrumb-item>
            </x-ui.breadcrumb-list>
        </x-ui.breadcrumb>
    </x-slot:header>
                        ...
                        <x-ui.card-content class="space-y-4">
                            <div class="grid grid-cols-2 gap-4">
                                <x-ui.field>
                                    ...
                                    <x-ui.input id="name" name="name" value="{{ old('name', $testimonial->name ?? '') }}" />
                                    <x-ui.field-error name="name" />
                                </x-ui.field>
                                <x-ui.field>
                                    <x-ui.field-label for="rating">Rating (1–5)</x-ui.field-label>
                                    <x-ui.input type="number" id="rating" name="rating" min="1" max="5" value="{{ old('rating', $testimonial->rating ?? 5) }}" />
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                <x-ui.field>
                                    ...
                                    <x-ui.input id="role" name="role" value="{{ old('role', $testimonial->role ?? '') }}" />
                                    <x-ui.field-error name="role" />
                                </x-ui.field>
                                <x-ui.field>
                                    <x-ui.field-label for="company">Company</x-ui.field-label>
                                    <x-ui.input id="company" name="company" value="{{ old('company', $testimonial->company ?? '') }}" />
                                    <x-ui.field-error name="company" />
                                </x-ui.field>
                            </div>
                            <x-ui.field>
                                <x-ui.field-label for="content">Review Content</x-ui.field-label>
                                <x-ui.rich-text-editor name="content">{!! old('content', $testimonial->content ?? '') !!}</x-ui.rich-text-editor>
                    ...
                    <x-ui.card>
                        ...
                        <x-ui.card-content class="space-y-4">
                            <div class="flex items-center justify-between">
                                ...
                                <x-ui.switch id="status" name="status" value="1" :checked="old('status', $testimonial->status ?? true)" />
                            </div>
                            <div class="flex items-center justify-between">
                                <x-ui.label for="is_featured" class="flex flex-col space-y-1">
                                    <span>Featured</span>
                                    <span class="font-normal text-xs text-muted-foreground">Highlight on homepage</span>
                                </x-ui.label>
                                <x-ui.switch id="is_featured" name="is_featured" value="1" :checked="old('is_featured', $testimonial->is_featured ?? false)" />
                            </div>
                        </x-ui.card-content>
                    </x-ui.card>
                </div>
