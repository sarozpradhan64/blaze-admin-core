<x-layouts.admin title="Tag: {{ $tag->name }}">
    <x-slot:header>
        <x-ui.breadcrumb>
            <x-ui.breadcrumb-list>
                ...
                <x-ui.breadcrumb-item><x-ui.breadcrumb-page>{{ $tag->name }}</x-ui.breadcrumb-page></x-ui.breadcrumb-item>
            </x-ui.breadcrumb-list>
        </x-ui.breadcrumb>
    </x-slot:header>
    ...
    <div class="flex items-center justify-between gap-4">
        <div>
            ...
            <p class="text-sm text-muted-foreground">{{ $usedItems->count() }}
                {{ Str::plural('item', $usedItems->count()) }} use this tag.</p>
        </div>
        <x-ui.button variant="outline" href="{{ route('admin.tags.manage') }}">
            <x-slot:before><x-lucide-arrow-left class="size-4" /></x-slot:before>
            Back to Tags
        </x-ui.button>
    </div>
            ...
            <div class="overflow-x-auto">
                <x-ui.table>
                    ...
                    <x-ui.table-body>
                        @forelse ($usedItems as $item)
                            <x-ui.table-row class="group">
                                ...
                                <x-ui.table-cell class="text-right">
                                    <x-ui.button variant="ghost" size="icon" href="{{ $item['url'] }}"
                        ...
                        @empty
                            <x-ui.table-row>
                                <x-ui.table-cell colspan="4" class="py-10 text-center text-muted-foreground">
