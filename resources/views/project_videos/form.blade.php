<x-layouts.admin title="{{ isset($video) ? 'Edit Video' : 'Add Video' }}">
    ...
    <div class="max-w-2xl">
        <h2 class="text-2xl font-bold tracking-tight mb-6">{{ isset($video) ? 'Edit Video' : 'Add Project Video' }}</h2>

        <form action="{{ isset($video) ? route('admin.project-videos.update', $video) : route('admin.project-videos.store') }}" method="POST" enctype="multipart/form-data">
            @csrf
                ...
                <x-ui.card-content class="space-y-4 pt-6">
                        ...
                        <x-ui.select name="project_id">
                            ...
                            <x-ui.select-content>
                                @foreach($projects as $project)
                                    <x-ui.select-item value="{{ $project->id }}" :selected="old('project_id', $video->project_id ?? $selectedProject) == $project->id">
                                        {{ $project->title }}
                                    </x-ui.select-item>
                                @endforeach
                            </x-ui.select-content>
                    ...
                    <x-ui.field>
                        ...
                        <x-ui.select name="video_type">
                            ...
                            <x-ui.select-content>
                                <x-ui.select-item value="youtube" :selected="old('video_type', $video->video_type ?? '') === 'youtube'">YouTube</x-ui.select-item>
                                <x-ui.select-item value="vimeo" :selected="old('video_type', $video->video_type ?? '') === 'vimeo'">Vimeo</x-ui.select-item>
                                <x-ui.select-item value="uploaded" :selected="old('video_type', $video->video_type ?? '') === 'uploaded'">Uploaded</x-ui.select-item>
                            </x-ui.select-content>
                        </x-ui.select>
                    ...
                    <x-ui.field>
                        ...
                        <x-ui.input id="video_url" name="video_url" value="{{ old('video_url', $video->video_url ?? '') }}" placeholder="https://www.youtube.com/watch?v=..." />
