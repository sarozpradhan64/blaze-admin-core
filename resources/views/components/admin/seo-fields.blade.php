            ...
            <div class="flex items-center justify-between">
                ...
                @if ($hasDefaults)
                    <x-ui.button type="button" variant="outline" size="sm" @click="fillDefaults()">
                        <x-lucide-wand class="size-3.5 mr-1.5" />
                        Auto-fill from defaults
                    </x-ui.button>
                @endif
            </div>
            <x-ui.card-description>Manage how this page appears in search engines and social media
                previews.</x-ui.card-description>
        </x-ui.card-header>
        <x-ui.card-content class="space-y-6">
            <div class="space-y-4">
                <h4 class="text-sm font-semibold text-muted-foreground uppercase tracking-wide">Search Engine</h4>

                <x-ui.field>
                    <x-ui.field-label for="seo_meta_title">Meta Title</x-ui.field-label>
                    <x-ui.input id="seo_meta_title" name="seo[meta_title]" placeholder="Defaults to page title if empty"
                        maxlength="70" x-model="metaTitle" />
                ...
                <x-ui.field>
                    ...
                    <x-ui.textarea id="seo_meta_description" name="seo[meta_description]" rows="3"
                        placeholder="Defaults to excerpt if empty" maxlength="160" x-model="metaDesc"></x-ui.textarea>
                    <x-ui.field-description>
            ...
            <div class="space-y-4">
                <h4 class="text-sm font-semibold text-muted-foreground uppercase tracking-wide">Open Graph (Facebook /
                    LinkedIn)</h4>

                <x-ui.field>
                    <x-ui.field-label for="seo_og_title">OG Title</x-ui.field-label>
                    <x-ui.input id="seo_og_title" name="seo[og_title]"
                        value="{{ old('seo.og_title', $seo?->og_title ?? '') }}" placeholder="Defaults to Meta Title" />
                </x-ui.field>

                <x-ui.field>
                    <x-ui.field-label for="seo_og_description">OG Description</x-ui.field-label>
                    <x-ui.textarea id="seo_og_description" name="seo[og_description]" rows="2"
                        placeholder="Defaults to Meta Description">{{ old('seo.og_description', $seo?->og_description ?? '') }}</x-ui.textarea>
                </x-ui.field>

                <x-ui.field>
                    <x-ui.field-label for="seo_og_image">OG Image URL</x-ui.field-label>
                    <x-ui.input id="seo_og_image" name="seo[og_image]" type="url"
