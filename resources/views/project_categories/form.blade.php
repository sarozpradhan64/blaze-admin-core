<x-layouts.admin title="{{ isset($category) ? 'Edit Category' : 'Add Category' }}">
    <x-slot:header>
        <x-ui.breadcrumb>
            <x-ui.breadcrumb-list>
                ...
                <x-ui.breadcrumb-item>
                    <x-ui.breadcrumb-link href="{{ route('admin.project-categories.index') }}">Categories</x-ui.breadcrumb-link>
                </x-ui.breadcrumb-item>
                <x-ui.breadcrumb-separator />
                <x-ui.breadcrumb-item>
    ...
    <div class="max-w-2xl">
        ...
        <form action="{{ isset($category) ? route('admin.project-categories.update', $category) : route('admin.project-categories.store') }}" method="POST">
            @csrf
            @if(isset($category))
                @method('PUT')
            @endif
            ...
            <div class="space-y-6">
                <x-ui.card>
                    ...
                    <x-ui.card-footer class="border-t bg-muted/50 flex justify-end gap-2 p-4">
                        <x-ui.button variant="outline" href="{{ route('admin.project-categories.index') }}">Cancel</x-ui.button>
