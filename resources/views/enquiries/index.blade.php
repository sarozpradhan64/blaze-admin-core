<x-layouts.admin title="Enquiries">
    ...
    <div class="grid grid-cols-2 gap-3 md:grid-cols-4">
        ...
        <div class="bg-card border border-border rounded-xl px-5 py-4 flex items-center gap-4">
            ...
            <div>
                <p class="text-xs text-muted-foreground font-medium uppercase tracking-wide">Total</p>
                <p class="text-2xl font-bold text-foreground leading-tight">{{ $stats['total'] }}</p>
            </div>
        </div>
        ...
        <div class="bg-card border border-border rounded-xl px-5 py-4 flex items-center gap-4">
            ...
            <div>
                <p class="text-xs text-muted-foreground font-medium uppercase tracking-wide">New</p>
                <p class="text-2xl font-bold text-primary leading-tight">{{ $stats['new'] }}</p>
            </div>
        </div>
        ...
        <div class="bg-card border border-border rounded-xl px-5 py-4 flex items-center gap-4">
            ...
            <div>
                <p class="text-xs text-muted-foreground font-medium uppercase tracking-wide">In Progress</p>
                <p class="text-2xl font-bold text-foreground leading-tight">{{ $stats['in_progress'] }}</p>
            </div>
        </div>

        {{-- Urgent --}}
        <div class="bg-card border border-border rounded-xl px-5 py-4 flex items-center gap-4">
            <div class="shrink-0 flex items-center justify-center size-10 rounded-lg bg-destructive/10 text-destructive">
                <x-lucide-alert-triangle class="size-5" />
            </div>
            <div>
                <p class="text-xs text-muted-foreground font-medium uppercase tracking-wide">Urgent</p>
                <p class="text-2xl font-bold text-destructive leading-tight">{{ $stats['urgent'] }}</p>
            </div>
        </div>
    </div>
    ...
    <form method="GET" action="{{ route('admin.enquiries.index') }}"
          class="flex flex-wrap gap-2 items-center bg-card border border-border rounded-xl px-4 py-3">
        {{-- Search --}}
        <div class="flex-1 min-w-52">
            <x-ui.input name="search" value="{{ request('search') }}" placeholder="Search by name, email or company…" class="h-9">
                <x-slot:leading><x-lucide-search class="size-4" /></x-slot:leading>
            </x-ui.input>
        </div>
            ...
            <x-ui.select name="status">
                ...
                <x-ui.select-content>
                    ...
                    @foreach(['new','in_progress','contacted','converted','closed','spam'] as $s)
                        <x-ui.select-item value="{{ $s }}" :selected="request('status') === $s">{{ ucfirst(str_replace('_',' ',$s)) }}</x-ui.select-item>
                    @endforeach
                </x-ui.select-content>
            </x-ui.select>
        </div>
        ...
        <div class="w-36">
            <x-ui.select name="priority">
                <x-ui.select-trigger class="h-9"><x-ui.select-value placeholder="All priorities" /></x-ui.select-trigger>
                <x-ui.select-content>
                    <x-ui.select-item value="">All priorities</x-ui.select-item>
                    @foreach(['low','normal','high','urgent'] as $p)
                        <x-ui.select-item value="{{ $p }}" :selected="request('priority') === $p">{{ ucfirst($p) }}</x-ui.select-item>
                    @endforeach
                </x-ui.select-content>
            </x-ui.select>
        </div>
        ...
        <div class="flex items-center gap-1.5 ml-auto">
            <x-ui.button type="submit" size="sm">
                <x-slot:before><x-lucide-filter class="size-3.5" /></x-slot:before>
                Filter
            </x-ui.button>
            @if(request()->hasAny(['search','status','priority']))
                <x-ui.button variant="ghost" size="sm" href="{{ route('admin.enquiries.index') }}">
                    <x-slot:before><x-lucide-x class="size-3.5" /></x-slot:before>
                    Clear
                </x-ui.button>
            @endif
        </div>
        ...
        <x-ui.card-content class="p-0">
            <x-ui.table>
                ...
                <x-ui.table-body>
                        ...
                        <x-ui.table-row class="{{ $enquiry->status === 'new' ? 'bg-primary/5' : '' }}">
                            ...
                            <x-ui.table-cell class="text-right">
                                <div class="flex justify-end gap-2">
                                    <x-ui.button variant="ghost" size="icon" href="{{ route('admin.enquiries.show', $enquiry) }}">
                    ...
                    @empty
                        <x-ui.table-row>
                            <x-ui.table-cell colspan="6" class="text-center py-10 text-muted-foreground">
                                No enquiries found.
                            </x-ui.table-cell>
                        </x-ui.table-row>
                    @endforelse
